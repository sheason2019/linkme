...
// 根据指定长度获取用户数量
func TableCountWithDate(tableName string, length int) ([]admin.SatisticsItem, error) {
	...
	for i := 0; i < length; i++ {
		...
		err = conn.
			Raw("select ? as label, count(id) as count from "+tableName+" where created_at < ?", specificTime.Format("01-02"), day).
			Scan(&item).
			Error
		if err != nil {
			return nil, err
